...
def makedir(owner, repository):
    dir_path = "Extracted data/" + owner + "/"+ repository + "/" + "Issues" + "/" + "date"  ##現バージョンではstargazersだが後々属性値を指定できるようにする
...
def export(file_name, dir_path):
    ...
    with open(os.path.join(dir_path, "csv",csv_filename), 'w', newline = '') as csvFile:
        ...
        csvwriter.writerow((['login'], ['cursor'], ['createdAt'],['closedAt']))
        for value in nodes:
