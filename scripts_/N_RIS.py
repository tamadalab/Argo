...
def main(arg, file_format, dir_path, write_data):
    ...
    for i in range(len(files)):
        ...
        if write_data is not None:
            fig_process.Print(label, dict_data[0], dict_data[1], write_data)
    ...
    for i in range(len(files)):
        os.makedirs(os.path.join(dir_path, arg[i]), exist_ok=True)
        fig_process.savefig(figure, os.path.join(dir_path, arg[i], "N_RIS"), file_format)
        fig_process.makedir(os.path.join("cache", arg[i], "N_RIS"))
