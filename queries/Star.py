...
def request(repository, dir_path, payload_1, endCursor, hasNextPage, file_num):
    print()
    ...
    if hasNextPage == 1:
        if endCursor != None:
            payload = (
                payload_1 + ",after:" + "\\\"" + endCursor + "\\\"" + payload_2
            )
        else:
...
def find(json_data, file_name, dir_path, data_cpl):
    list_of_files = glob.glob(
        os.path.join(dir_path, "json", "*.json")
    )  # list型 指定したフォルダからファイル一覧を取得
    ...
    hasNextPage = json_dict["data"]["repository"]["stargazers"]["pageInfo"][
        "hasNextPage"
    ]
...
def main(repository, make_path, dir_stored):
    ...
    for i in range(len(repository)):
        # Make File, Payload
        repo_data = FileMake.input_repository(repository[i])  ## owner, repository
        if make_path == True:
...
if __name__ == "__main__":
    repositories = sys.argv[1:] ## owner/repositoryはrepository[1]以降に格納
    dir_path = True
    dir_stored = False
