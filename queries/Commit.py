...
def request(repository, dir_path, payload_1, end_cursor, has_next_page, file_num):
    ...
    while has_next_page:
        ...
        headers = {
            "Authorization": "bearer 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",
...
def export(file_name, dir_path):
    ...
    for egde in data["data"]["repository"]["defaultBranchRef"]["target"]["history"]["edges"]:
        ...
        created_utc = committedDate.replace("Z", "")
        d = datetime.datetime.fromisoformat(created_utc).date()
        committedDate = str(d.year) + "-" + str(d.month) + "/" + str(d.day)


        temp_n = [committedDate, commitUrl]
        nodes.append(temp_n)
    ...
    with open(os.path.join(dir_path, "csv", csv_filename), "w", newline="") as csvFile:
        ...
        csvwriter.writerow(["committedDate", "commitUrl"])
...
def main(repository, make_path, dir_stored):
    ...
    for i in range(len(repository)):
        ...
        if make_path:
            dir_path = FileMake.makedir(repo_data[0], repo_data[1], "Commit")
        else:
