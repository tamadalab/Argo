...
def makedir(owner, repository):
    dir_path = "Extracted data/" + owner + "/"+ repository + "/" + "PullRequest" + "/" + "date"  ##現バージョンではstargazersだが後々属性値を指定できるようにする
...
def export(file_name, dir_path):
    ...
    for node in data["data"]["repository"]["pullRequests"]["nodes"]:
        ...
        if not mergedAt:
            ...
            mergedt = str(d_m.year) + "-" + str(d_m.month) + "/" + str(d_m.day)
        temp_n = [title, createdt, merged, mergedt]
        nodes.append(temp_n)
    ...
    with open(os.path.join(dir_path, "csv",csv_filename), 'w', newline = '') as csvFile:
        ...
        csvwriter.writerow((['title'], ['createdAt'], ["merged"], ["mergedAt"]))
        for value in nodes:
